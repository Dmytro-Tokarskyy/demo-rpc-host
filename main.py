...
def main():
    ...
    adb.send_intent(DEVICE_1_SERIAL, ACTION_DISCOVER)
    device1_ui = uiautomator2.connect(DEVICE_1_SERIAL)
    device1_ui(resourceId=OK_BUTTON1).click()

    adb.send_intent(DEVICE_2_SERIAL, ACTION_DISCOVER)
    ...
    adb.send_intent(DEVICE_1_SERIAL, ACTION_PAIR, DEVICE_2_MAC)

    # # Accept pairing on devices
    device1_ui(resourceId=OK_BUTTON1).click()
    device2_ui(resourceId=OK_BUTTON2).click()

    adb.send_intent(DEVICE_1_SERIAL, ACTION_UNPAIR, DEVICE_2_MAC)
    adb.send_intent(DEVICE_2_SERIAL, ACTION_UNPAIR, DEVICE_1_MAC)
